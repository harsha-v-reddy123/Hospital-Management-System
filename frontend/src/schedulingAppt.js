...
const ScheduleDisplay = ({ schedule }) => {
  ...
  return (
    <Box>
      <Heading level={2}>Doctor's Schedule</Heading>
      {orderedDays.map(day => (
        <Box key={day}>
          <Heading level={3}>{day}</Heading>
          <Box>
            {groupedSchedule[day]?.map((slot, index) => (
              <Box key={index} direction="row" gap="small">
                <Text>{slot.starttime} - {slot.endtime}, Break time-{slot.breaktime}</Text>
              </Box>
            ))}
          </Box>
        </Box>
...
const DoctorsDropdown = ({ onSelectDoctor }) => {
...
export class SchedulingAppt extends Component {
  ...
  fetchDoctorSchedule = (doctorId) => {
    fetch(`http://localhost:3001/showSchedule?doctorId=${doctorId}`)
      .then(res => res.json())
      .then(res => {
    ...
    return (
      <Grommet theme={theme} full>
        ...
        <Box direction="row">
            ...
            <Box align="center" pad="small" gap="small">
              ...
              >
                <Box align="center" gap="small">
                  <DoctorsDropdown onSelectDoctor={this.fetchDoctorSchedule} />
                </Box>
                <DateTimeDropButton />
                <ConcernsTextArea />
                <br />
          ...
          <Box width="30%">
            <ScheduleDisplay schedule={doctorSchedule} />
          </Box>
        </Box>
      </Grommet>
    );
