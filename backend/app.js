  ...
  con.query(sql_statement, function (error, results, fields) {
    if (error) {
      ...
      if (results.length === 0) {
        ...
        password_in_use = password;
        who = "pat";
        let loginTime = getCurrentTimeInIST();
        let listKey = `user:${email}:loginTimes`;

        // Store email and login time in Redis
        redisClient.rpush(listKey, loginTime, (err, reply) => {
          if (err) {
            console.error('Redis RPUSH error:', err);
            return res.status(500).json({ failed: 'Redis operation failed' });
          }
          console.log(`Login time added to Redis list for ${email}. List length now: ${reply}`);
          });
      }
      return res.json({
        data: results
...
// Endpoint to show the schedule of a doctor
app.get('/showSchedule', (req, res) => {
  let doctorId = req.query.doctorId; // Assuming doctorId is passed as a query parameter

  // Query to fetch the schedule of the doctor
  let statement = `
    SELECT *
    FROM Schedule
    WHERE id IN (
